...
const whereaboutsController = {
    ...
    checkUserExists: async (req, res, next) => {
        try {
            ...
            const passwordIsMatch = await bcrypt.compare(password, existingUser.rows[0].password);
            if (!passwordIsMatch) {
                return next({
    ...
    getContacts: async (req, res, next) => {
        try {
            res.locals.contacts = await db.query(`select u.phone_number, u.name from users u
                inner join contacts_join cj on u.phone_number = cj.contact_phone_number
                where cj.traveler_phone_number = $1`, [req.params['phone_number']]);
            return next();
        }
        catch (error) {
            return next({
    ...
    getUserByPhoneNumber: async (req, res, next) => {
        try {
            res.locals.user = await db.query(`SELECT * FROM users WHERE phone_number=$1`, [req.params['phone_number']]);
            return next();
        }
        catch (error) {
            return next({
    ...
    deleteContact: async (req, res, next) => {
        try {
            await db.query(`DELETE FROM contacts_join
                WHERE traveler_phone_number = $1 AND contact_phone_number = $2`, [req.params.travelerPhone, req.params.contactPhone]);
            return next();
        }
        catch (error) {
            return next({
    ...
    startNewTrip: async (req, res, next) => {
        try {
    ...
    myTrip: async (req, res, next) => {
        try {
            res.locals.trip = await db.query(`SELECT *
          FROM trips t
