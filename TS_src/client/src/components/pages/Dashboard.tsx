        ...
        {trips.map((trip) => (
          <div>Trip Id: {trip.id} | Trip Start Time: {trip.start_timestamp} ||</div>
			...
			<div className="functions-container">
				...
				{activeComponent === 'myTripCard' && <MyTripCard userInfo={userInfo} setUserInfo={setUserInfo} userTrip={userTrip} setUserTrip={setUserTrip} />}
				{activeComponent === 'tripsImWatching' && <TripImWatching userInfo={userInfo} />}
				{activeComponent === 'chatPage' && <ChatPage userInfo={userInfo} setUserInfo={setUserInfo} path="/chat" socket={socket} />}
			</div>
		</div>
	);
}
