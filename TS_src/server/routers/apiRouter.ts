...
router.get('/users/contacts/', whereaboutsController.getContacts, (req: Request, res: Response) => {
...
router.get('/users/:phone_number', whereaboutsController.getUserByPhoneNumber, (req: Request, res: Response) => {
	const { rows } = res.locals.user;
	res.status(200).json(rows);
});

//delete a contact
router.delete('/users/contacts/traveler/:travelerPhone/contact/:contactPhone', whereaboutsController.deleteContact, (req: Request, res: Response) => {
	res.status(204).json([]); //204 --> no content
});

router.post('/register', whereaboutsController.insertNewUser, (req: Request, res: Response) => res.sendStatus(200));
...
//get my current trip
router.get('/trips/my/:phone_number', whereaboutsController.myTrip, (req: Request, res: Response) => {
